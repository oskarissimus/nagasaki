...
def test_multiple_own_orders_should_cancel_all_and_create_one(initialized_state: State):
    ...
    assert bitclude_client.cancel_and_wait.call_args[0][0] is not None

    result_cancel_orders = {
        args_list[0][0] for args_list in bitclude_client.cancel_and_wait.call_args_list
    }
    expected_cancel_orders = [
        offer.to_order_maker() for offer in state.bitclude.active_offers
    ]
    expected_cancel_orders = set(expected_cancel_orders)

    assert result_cancel_orders == expected_cancel_orders
